    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                {{ __('Edit Laporan: ') }} <span class="text-blue-600">{{ $ticket->ticket_number }}</span>
            </h2>
        </div>
    </x-slot>
            ...
            <div class="bg-white overflow-hidden shadow-lg sm:rounded-xl border border-gray-100">
                <div class="p-6 md:p-8">
                    ...
                    <div class="mb-8 border-b pb-4">
                        <h3 class="text-2xl font-extrabold text-gray-800">Revisi Detail Laporan</h3>
                        <p class="text-gray-500 text-sm mt-1">Gunakan form ini untuk memperbaiki kesalahan ketik atau mengubah kategori laporan.</p>
                    </div>
                    ...
                    <form action="{{ route('tickets.update', $ticket->id) }}" method="POST">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                            ...
                            <div class="bg-blue-50/50 p-6 rounded-xl border border-blue-100">
                                ...
                                <div class="mb-2">
                                    ...
                                    <input type="text" name="location" value="{{ old('location', $ticket->location) }}" required class="w-full border-gray-300 rounded-lg shadow-sm focus:ring-blue-500 focus:border-blue-500 bg-white">
                                </div>
                            </div>

                            <div class="bg-amber-50/50 p-6 rounded-xl border border-amber-100">
                                <h4 class="text-sm font-bold text-amber-600 uppercase tracking-wider mb-5 flex items-center">
                                ...
                                <div class="mb-5">
                                    ...
                                    <input type="text" name="title" value="{{ old('title', $ticket->title) }}" required class="w-full border-gray-300 rounded-lg shadow-sm focus:ring-amber-500 focus:border-amber-500 bg-white">
                                </div>

                                <div class="mb-2">
                                    <label class="block text-sm font-bold text-gray-700 mb-1">Deskripsi Lengkap <span class="text-red-500">*</span></label>
                                    <textarea name="description" rows="5" required class="w-full border-gray-300 rounded-lg shadow-sm focus:ring-amber-500 focus:border-amber-500 bg-white">{{ old('description', $ticket->description) }}</textarea>
                                </div>
                            </div>
                        ...
                        <div class="mt-8 flex items-center justify-end gap-4 border-t pt-6">
                            <a href="{{ route('tickets.show', $ticket->id) }}" class="text-gray-500 hover:text-gray-800 font-bold text-sm transition duration-150 py-2 px-4 rounded-lg hover:bg-gray-100">
                                Batal & Kembali
                            </a>
                            <button type="submit" class="bg-amber-500 hover:bg-amber-600 text-white font-bold py-3 px-8 rounded-lg shadow-lg shadow-amber-500/30 transition duration-200 flex items-center transform hover:-translate-y-0.5">
